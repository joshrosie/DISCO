...
def edm_sampler(
    ...
    init_clean: dict[str, torch.Tensor] | None = None,
    ...
    # SDEdit / diffusion-editing: start the reverse process from a known clean
    # structure noised to sigma_start, integrating a truncated Karras schedule
    # over [sigma_min, sigma_start]. sigma_min/sigma_max passed to the denoiser
    # stay at the model's training values (preconditioning), only the schedule
    # endpoint changes.
    sigma_max_eff = float(sigma_max) if sigma_start is None else float(sigma_start)
    ...
    if init_clean is not None:
        # x_sigma = x0 + sigma_start * eps  (t_steps[0] == sigma_max_eff == sigma_start)
        type_next = init_clean["type"].to(device=device, dtype=type_x.dtype) + type_x * t_steps[0]
        frac_next = init_clean["frac"].to(device=device, dtype=frac_x.dtype) + frac_x * t_steps[0]
        lat_next = init_clean["lat"].to(device=device, dtype=lat_x.dtype) + lat_x * t_steps[0]
    else:
    ...
    # Optional capture of the noisy iterate at each step for trajectory animation.
    trajectory: list[dict[str, torch.Tensor]] | None = [] if save_trajectory else None

    def _record(sigma_val) -> None:
        if trajectory is None:
            return
        trajectory.append(
            {
                "sigma": float(sigma_val),
                "type": type_next.detach().to("cpu").clone(),
                "frac": frac_next.detach().to("cpu").clone(),
                "lat": lat_next.detach().to("cpu").clone(),
            }
        )

    _record(t_steps[0])

    def _denoise(type_in, frac_in, lat_in, sigma_in):
        if denoise_fn is not None:
            return denoise_fn(type_in, frac_in, lat_in, sigma_in)
        return denoise_edm(
            model=model,
            type_noisy=type_in,
            frac_noisy=frac_in,
    ...
    for i, (t_cur, t_next_val) in enumerate(zip(t_steps[:-1], t_steps[1:])):
        ...
        denoised = _denoise(type_hat, frac_hat, lat_hat, sigma_hat)
        ...
        if i < num_steps - 1:
            ...
            denoised_next = _denoise(type_next, frac_next, lat_next, sigma_next)
        ...
        if save_trajectory and (i % trajectory_every == 0 or i == num_steps - 1):
            _record(t_next_val)
    ...
    out = {"type": type_next, "frac": frac_next, "lat": lat_next}
