...
class CrystaliteModel(nn.Module):
    ...
    def forward(
        ...
        time_cond_type: torch.Tensor | None = None,
        ...
        Args:
            ...
            time_cond: (B,) scalar conditioning value passed to TimeEmbedder
                (geometry/global schedule).
            ...
            noise_level: optional (B,) positive noise scale used only by the
                geometry/noise gate path. EDM passes sigma here; VFM leaves it unset.
            time_cond_type: optional (B,) separate time conditioning for the type
                modality. When provided, TimeEmbedder receives (time_cond, time_cond_type)
                instead of (time_cond, time_cond). Used by EDM-CatFlow hybrid where
                geometry uses EDM sigma and types use CatFlow interpolation time.
        """
        ...
        t_second = time_cond if time_cond_type is None else time_cond_type
        t_emb = self.time(time_cond, t_second)
