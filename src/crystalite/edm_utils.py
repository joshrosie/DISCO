...
def denoise_edm(
    ...
    catflow_time: torch.Tensor | None = None,
    ...
    with amp_ctx:
        try:
            raw = model(
                ...
                noise_level=sigma,
                time_cond_type=catflow_time,
            )
        except TypeError as exc:
            # Backward compatibility for pre-VFM Crystalite/MP20 EDM signatures:
            #   forward(..., t_sigma, lattice_bias_feats=None)
            # Older models recover sigma internally for GEM, so the positional
            # t_sigma call preserves the original EDM behavior.
            msg = str(exc)
            if "time_cond" not in msg and "noise_level" not in msg:
                raise
            raw = model(
                type_in,
                frac_in,
