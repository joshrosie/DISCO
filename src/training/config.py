...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        choices=["none", "synthetic_concat", "oversample_real"],
        help="Training-data augmentation mode for synthetic Crystalite sidecar experiments.",
    )
    parser.add_argument(
        "--synthetic_data",
        type=str,
        default=None,
        help=(
            "Synthetic dataset root directory (containing raw/train.csv) used "
            "when --augmentation synthetic_concat. Loaded via MP20Tokens(split='train'). "
            "Multiple roots can be provided as an os.pathsep-separated list."
        ),
    )
    parser.add_argument(
        "--num_extra_samples",
        type=int,
        default=None,
        help="Number of replacement-sampled real examples appended for --augmentation oversample_real.",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--sample_full_train_novelty",
        action="store_true",
        help=(
            "Also log sample/train_unique_rate, sample/train_novel_rate, and "
            "sample/train_un_rate against the actual training dataset after "
            "augmentation. This can be expensive for large synthetic runs."
        ),
    )
    parser.add_argument(
        "--precise_every",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to a checkpoint .pt; load model + EMA state_dicts and resume the step "
            "counter from ckpt['step']. Optimizer/scheduler state are NOT loaded (fresh)."
        ),
    )
    parser.add_argument(
        "--best_ckpt",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "auto: DNG prefers Train_MSUN when full-train novelty is enabled, "
            "then raw MSUN; CSP uses val MR/RMSE ratio with sample-first tag order. "
            "legacy restores previous DNG sample-first raw-MSUN ordering."
        ),
    )
    # Loss weights
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Legacy compatibility flag. Crystalite DNG no longer supports discrete "
            "atom-type diffusion; the option is kept only so old launch scripts fail "
            "with a clear error."
        ),
    )
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "Softmax temperature applied to type logits when decoding atom types "
            "during sampling. Values > 1 increase diversity (higher novelty at the "
            "cost of composition quality); 1.0 is standard argmax-equivalent greedy "
            "decoding. Only used in train_atom_prior.py."
        ),
    )
    parser.add_argument(
        "--sample_logit_scale",
        ...
        default=10.0,
        help=(
            "Scalar multiplier applied to cosine similarity scores before temperature "
            "softmax when decoding atom types stochastically. Higher values sharpen "
            "the distribution, making temperature more effective. Only applies when "
            "sample_temperature != 1.0 and the encoding uses a snap_table "
            "(e.g. chem_pca_v2_16). Only used in train_atom_prior.py."
        ),
    )
    parser.add_argument(
        "--csp_ckpt",
        ...
        default=None,
        help=(
            "Path to a CSP checkpoint. When provided during atom-prior training, "
            "generated compositions are passed to the CSP model to produce full "
            "structures, enabling structure-level validity metrics during sampling."
        ),
    )
    parser.add_argument(
        "--csp_sample_num_steps",
        type=int,
        default=None,
        help="Denoising steps for CSP geometry generation (defaults to --sample_num_steps).",
    )
    return parser
...
def validate_args(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    ...
    if args.augmentation == "synthetic_concat" and not args.synthetic_data:
        parser.error("--synthetic_data is required when --augmentation synthetic_concat.")
    if args.augmentation != "synthetic_concat" and args.synthetic_data:
        parser.error("--synthetic_data is only valid with --augmentation synthetic_concat.")
    if args.augmentation == "oversample_real":
        if args.num_extra_samples is None or int(args.num_extra_samples) < 0:
            parser.error("--num_extra_samples >= 0 is required with --augmentation oversample_real.")
    elif args.num_extra_samples is not None and int(args.num_extra_samples) < 0:
        parser.error("--num_extra_samples must be >= 0 when provided.")
