...
def patch_nequip_torchsim_calc_neighborlist_backend(calculator, backend: str = "vesin"):
    """Override the compiled NequIP calc's neighbor-list backend.

    The compiled TorchSim calc hard-defaults its NL backend to ``alchemiops``,
    which is broken against the installed nvalchemiops (``batch_cell_list`` was
    refactored from a callable into a module, so nequip's ``alchemiops_nl(...)``
    raises "module object is not callable" and every batched relaxation fails).
    ``vesin`` is a GPU/torch backend like alchemiops. No-op when backend is
    ``alchemiops`` or when the calc exposes no patchable transforms.
    """

    if backend == "alchemiops":
        return calculator
    for t in getattr(calculator, "transforms", []) or []:
...
def make_nequip_batch_calculator(compiled_model: str, stability_device: str):
    ...
    try:
        with warnings.catch_warnings():
            ...
            calc = patch_nequip_torchsim_calc_neighborlist_backend(calc)
    finally:
        # NequIP compiled model loading mutates torch global default dtype.
        torch.set_default_dtype(prev_default_dtype)
    return calc, stability_device
