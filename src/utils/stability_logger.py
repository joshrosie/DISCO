...
class StabilityLogger:
    ...
    def _thermo_exception_reason(exc: Exception) -> tuple[str, bool]:
        ...
        if "chemical species" in message or "atom_type_map" in message:
            return "unsupported_element", False
        if "number of neighbors" in message or "neighboroverflow" in message:
            return "relax_neighbor_overflow", False
    ...
    def _is_structure_level_batch_exception(exc: Exception) -> bool:
        message = str(exc).strip().lower()
        return (
            ("force divergence" in message)
            or ("non-finite" in message)
            or ("chemical species" in message)
            or ("atom_type_map" in message)
            or ("number of neighbors" in message)
            or ("neighboroverflow" in message)
        )
