...
from src.data.synthetic_augmentation import build_augmented_train_dataset
...
from src.utils.sample_viz import build_ase_view
from src.utils.stability_logger import StabilityLogger, _ThermoConfig
from src.utils.type_metrics import compute_type_distribution_stats
...
def _validate_crystalite_args(args: Any, parser: Any) -> None:
    if args.discrete_types and not args.csp:
        parser.error(
            "--discrete_types is no longer supported for Crystalite DNG training. "
            "Use the continuous EDM type path or train_atom_prior_v2.py for D3PM atom-type work."
        )
...
def main() -> None:
    ...
    _validate_crystalite_args(args, parser)
    nmax = args.nmax
    ...
    ds, dataset_composition = build_augmented_train_dataset(
        real_train_ds,
        augmentation_mode=args.augmentation,
        synthetic_data=args.synthetic_data,
        num_extra_samples=args.num_extra_samples,
        seed=args.seed,
        nmax=nmax,
    )
    ...
    print(
        "[augmentation]",
        " ".join(f"{k}={v}" for k, v in dataset_composition.items()),
    )
    same_metrics_source = (
    ...
    if getattr(args, "init_from", None):
        ...
        print(f"[init-from] loading model + EMA from {init_path}")
        init_ckpt = torch.load(init_path, map_location=device, weights_only=False)
        model.load_state_dict(init_ckpt["model_state_dict"])
        if ema is not None and "ema_state_dict" in init_ckpt:
            ema.load_state_dict(init_ckpt["ema_state_dict"])
        start_step = int(init_ckpt.get("step", 0))
        scheduler.last_epoch = start_step
        print(
            f"[init-from] resumed at step={start_step}; optimizer state is fresh; "
            f"LR scheduler fast-forwarded"
        )
    ...
    log_metrics(
        {
            ...
            "dataset_splits/real_train_count": dataset_composition["real_train_count"],
            "dataset_splits/synthetic_train_count": dataset_composition["synthetic_train_count"],
            "dataset_splits/oversampled_real_count": dataset_composition["oversampled_real_count"],
            "dataset_splits/effective_train_count": dataset_composition["effective_train_count"],
            "dataset_splits/num_params": num_params,
            "dataset/name": args.dataset_name,
            "dataset/nmax": nmax,
            "dataset/augmentation_mode": args.augmentation,
            "dataset/synthetic_metadata_path": dataset_composition["synthetic_metadata_path"],
    ...
    ase_view = build_ase_view(args.sample_vis_count)
    ...
    if not args.csp:
        ...
        if enable_dng_metrics:
            ...
            if bool(getattr(args, "sample_full_train_novelty", False)):
                print(
                    "[eval] building full training novelty reference from "
                    f"{len(ds)} augmented train structures"
                )
                full_train_novelty_ref_structs = dataset_to_structures(ds)
                print(
                    "[eval] full training novelty reference size: "
                    f"{len(full_train_novelty_ref_structs)}"
                )
    ...
    sampling_ctx = SamplingContext(
        ...
        full_train_novelty_ref_structs=full_train_novelty_ref_structs,
    ...
    for step in progress:
        ...
        if step % args.log_every == 0:
            ...
            if real_mask.any():
                ...
                type_stats = compute_type_distribution_stats(denoised["type"], pad_mask)
                metrics["stats/type_entropy"] = type_stats["type_entropy"]
                metrics["stats/type_entropy_norm"] = type_stats["type_entropy_norm"]
                metrics["stats/type_max_prob"] = type_stats["type_max_prob"]
                ...
                metrics["stats/type_entropy"] = 0.0
                metrics["stats/type_entropy_norm"] = 0.0
                metrics["stats/type_max_prob"] = 0.0
