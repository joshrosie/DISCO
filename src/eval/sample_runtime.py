...
def _resolve_sampler_name(args: Any) -> str:
    name = str(getattr(args, "sampler", "edm")).strip().lower()
    if name != "edm":
        raise ValueError(f"Unsupported sampler {name!r}. Only 'edm' is supported.")
    return name


def _resolve_model_type_dim(model: torch.nn.Module, type_encoding: Any) -> int:
    return int(getattr(model, "type_dim", type_encoding.type_dim))


def _sample_chunk_items(
    *,
    ctx: "SamplingContext",
    pad_mask_chunk: torch.Tensor,
    generator: torch.Generator,
    fixed_atom_types: torch.Tensor | None,
    skip_type_scaling: bool,
    allowed_mask: torch.Tensor | None,
    source_atom_idx: torch.Tensor | None,
) -> tuple[list[dict[str, torch.Tensor]], torch.Tensor, torch.Tensor]:
    args = ctx.args
    _resolve_sampler_name(args)
    samples = edm_sampler(
        model=ctx.model,
        pad_mask=pad_mask_chunk,
        type_dim=_resolve_model_type_dim(ctx.model, ctx.type_encoding),
    ...
    if source_atom_idx is not None:
        atom_idx = source_atom_idx.to("cpu")
    else:
    ...
    return items, atom_idx, pad_mask_cpu
...
def _has_safe_min_distance(struct, cutoff: float = 0.3) -> bool:
    """Reject structures with atoms closer than cutoff (Angstroms)."""
    try:
        if len(struct) < 2:
            return True
        dist = struct.distance_matrix
        np.fill_diagonal(dist, np.inf)
        return float(dist.min()) >= cutoff
    except Exception:
        return False


def _collect_thermo_structures_with_fallback(
    sample_items: list[dict[str, Any]],
    *,
    pred_crys_list: list[Any] | None,
    count: int,
) -> list[Any]:
    if count <= 0:
        return []
    ...
    limit = min(int(count), len(sample_items))

    if pred_crys_list:
        max_pred_items = min(len(pred_crys_list), limit)
        for idx in range(max_pred_items):
            crys = pred_crys_list[idx]
            if not getattr(crys, "constructed", False):
                continue
            struct = getattr(crys, "structure", None)
            if struct is None:
                continue
            if not _has_safe_min_distance(struct):
                continue
            thermo_structs.append(struct)
    ...
    from src.data.mp20_tokens import tokens_to_structure as _tokens_to_structure

    for idx, item in enumerate(sample_items[:limit]):
        if idx in used_item_indices:
            continue
        try:
            struct = _tokens_to_structure(item)
            if not _has_safe_min_distance(struct):
                continue
            thermo_structs.append(struct)
        except Exception:
...
def _maybe_log_standalone_thermo(
    *,
    sample_items: list[dict[str, Any]],
    pred_crys_list: list[Any] | None,
    ...
    thermo_count = (
        ...
        else request.metrics_count
    )
    thermo_count = min(int(thermo_count), int(n_samples))
    if thermo_count <= 0:
        return

    if request.is_csp:
        # Preserve legacy CSP thermo semantics from csp_dev: evaluate the first
        # thermo_count raw samples in-order, without prefiltering through the
        # Crystal evaluator. This keeps NequIP batch composition stable across
        # branches.
        thermo_structs = []
        from src.data.mp20_tokens import tokens_to_structure as _tokens_to_structure
        ...
        thermo_structs = _collect_thermo_structures_with_fallback(
            sample_items,
    ...
    if ctx.ref_structs:
        ...
        if ref_count > 0:
            ...
            if compare_payload:
                log_metrics(compare_payload, step=step, enabled=ctx.wandb_enabled)
...
class SamplingContext:
    ...
    full_train_novelty_ref_structs: list[Any] | None
    ...
    with torch.no_grad():
        for start in range(0, n_samples, chunk_size):
            ...
            if request.is_csp:
                ...
                source_atom_idx_chunk = a0_chunk
            ...
            if getattr(args, "discrete_types", False) and not request.is_csp:
                raise ValueError(
                    "Crystalite DNG sampling no longer supports --discrete_types. "
                    "Use the continuous EDM type path or atom-prior D3PM instead."
                )
            items_chunk, atom_idx, pad_mask_cpu_chunk = _sample_chunk_items(
                ctx=ctx,
                pad_mask_chunk=pad_mask_chunk,
                generator=generator,
                fixed_atom_types=fixed_atom_types,
                skip_type_scaling=request.is_csp,
                allowed_mask=ctx.train_allowed_mask,
                source_atom_idx=source_atom_idx_chunk,
            )
            sample_items.extend(items_chunk)
...
def _generate_csp_items_for_indices(
    ...
    with torch.no_grad():
        for start in range(0, len(target_indices), chunk_size):
            ...
            items_chunk, _, _ = _sample_chunk_items(
                ctx=ctx,
                pad_mask_chunk=pad_mask_chunk,
                generator=generator,
                fixed_atom_types=fixed_atom_types,
...
def evaluate_dng_sampling_batch(
    ...
    if evaluator_result.eval_count > 0:
        ...
        if evaluator_result.dist_metrics:
            ...
            if "sample_dist/wdist_density_mass" in evaluator_result.dist_metrics:
                dist_payload[f"{tag}/wdist_density_mass"] = (
                    evaluator_result.dist_metrics["sample_dist/wdist_density_mass"]
                )
            if not tag.startswith("precise"):
                dist_payload.update(
                    {
    ...
    if ctx.full_train_novelty_ref_structs:
        train_novelty_result = compute_novelty_metrics(
            sample_items,
            ctx.full_train_novelty_ref_structs,
            limit=metrics_count,
            minimum_nary=1,
        )
        if train_novelty_result.novelty_metrics:
            log_metrics(
                {
                    f"{tag}/train_unique_rate": float(
                        train_novelty_result.unique_rate or 0.0
                    ),
                    f"{tag}/train_novel_rate": float(
                        train_novelty_result.novel_rate or 0.0
                    ),
                    f"{tag}/train_un_rate": float(
                        train_novelty_result.un_rate or 0.0
                    ),
                },
                step=step,
                enabled=ctx.wandb_enabled,
            ...
            if ctx.thermo_logger is not None and step >= args.no_thermo_before_steps:
                ...
                if sun_target > 0:
                    train_sun_result = compute_sun_metrics(
                        train_novelty_result.novelty_metrics,
                        thermo_logger=ctx.thermo_logger,
                        tag=f"{tag}/train",
                        step=step,
                        enabled=ctx.wandb_enabled,
                    ...
                    if train_sun_result.thermo_metrics:
                        log_metrics(
                            train_sun_result.thermo_metrics,
                            step=step,
                            enabled=ctx.wandb_enabled,
                        )
                    if train_sun_result.summary_metrics:
                        log_metrics(
                            train_sun_result.summary_metrics,
                            step=step,
                            enabled=ctx.wandb_enabled,
                        )
                    train_msun = train_sun_result.dng_payload.get("MSUN")
                    if train_msun is not None:
                        outcome.dng_payload["Train_MSUN"] = float(train_msun)

    _maybe_log_standalone_thermo(
        sample_items=sample_items,
        pred_crys_list=pred_crys_list,
        request=request,
        ctx=ctx,
