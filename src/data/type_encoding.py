...
class AtomicNumberEncoding:
    ...
    def __post_init__(self) -> None:
        ...
        # Channels: elements 1..VZ (indices 0..vz-1), pad token (index vz), MASK token (index vz+1).
        self.mask_idx = int(self.vz) + 1
        self.type_dim = int(self.vz) + 2
...
def _sample_from_scores(
    ...
    *,
    ...
    return torch.where(~pad_mask.bool(), sampled, torch.zeros_like(sampled)).long()


def sample_logits_to_A0(
    ...
    *,
    ...
    if isinstance(type_encoding, AtomicNumberEncoding):
        elem_scores = type_logits[..., : type_encoding.vz].clone()
        if elem_scores.shape[-1] != type_encoding.vz:
            raise ValueError(
                "AtomicNumberEncoding expects element-channel logits with "
                f"last dimension {type_encoding.vz}, got {elem_scores.shape[-1]}."
            )
        if allowed_mask is not None:
            allowed = allowed_mask.to(device=elem_scores.device, dtype=torch.bool).reshape(-1)
            if allowed.numel() != type_encoding.vz:
                raise ValueError(
                    f"allowed_mask must have length {type_encoding.vz}, got {allowed.numel()}"
                )
            if bool(allowed.any()):
                elem_scores[..., ~allowed] = -1e9
    ...
    if isinstance(type_encoding, PeriodicTable2DEncoding):
        row_logits = type_logits[..., : type_encoding.num_row]
        col_logits = type_logits[..., type_encoding.num_row :]
        if col_logits.shape[-1] != type_encoding.num_col:
            raise ValueError(
                f"Expected {type_encoding.num_col} column logits, got {col_logits.shape[-1]}."
            )
        device = type_logits.device
        valid_cells = type_encoding.valid_cells.to(device=device)
        pair_scores = row_logits[..., :, None] + col_logits[..., None, :]
        pair_mask = valid_cells
        if allowed_mask is not None:
            allowed_cells = type_encoding._allowed_cells_from_elements(allowed_mask, device=device)
            if bool(allowed_cells.any()):
                pair_mask = valid_cells & allowed_cells
        if not bool(pair_mask.any()):
            pair_mask = valid_cells
        pair_scores = pair_scores.masked_fill(~pair_mask, -1e9)
        flat_scores = pair_scores.reshape(*pair_scores.shape[:-2], -1)
        flat_idx = _sample_from_scores(
            flat_scores,
            pad_mask=pad_mask,
            temperature=temperature,
            generator=generator,
        )
        flat_idx = (flat_idx - 1).clamp_min(0)
        row_idx = flat_idx // type_encoding.num_col
        col_idx = flat_idx % type_encoding.num_col
        row_col_to_z = type_encoding.row_col_to_z.to(device=device)
        atom_idx = row_col_to_z[row_idx, col_idx]
        return torch.where(~pad_mask.bool(), atom_idx, torch.zeros_like(atom_idx)).long()

    if hasattr(type_encoding, "snap_table"):
        device = type_logits.device
        snap_table = type_encoding.snap_table.to(device=device)
        # Normalize to cosine similarity so temperature scaling is meaningful.
        # Raw dot products are flat for PCA/chemistry encodings, making softmax
        # nearly uniform. Argmax (decode path) is scale-invariant so it's unaffected.
        type_logits_norm = F.normalize(type_logits, p=2, dim=-1)
        elem_scores = (type_logits_norm @ snap_table.T) * float(logit_scale)
        if allowed_mask is not None:
            allowed = allowed_mask.to(device=device, dtype=torch.bool).reshape(-1)
            if allowed.numel() != int(type_encoding.vz):
                raise ValueError(
                    f"allowed_mask must have length {type_encoding.vz}, got {allowed.numel()}"
                )
            allowed = allowed[: elem_scores.shape[-1]]
            if bool(allowed.any()):
                elem_scores = elem_scores.masked_fill(~allowed, -1e9)
        return _sample_from_scores(
            elem_scores,
            pad_mask=pad_mask,
            temperature=temperature,
    ...
    raise TypeError(f"Unsupported type encoding for sampling: {type(type_encoding).__name__}")
