...
def process_one(
    ...
    try:
        ...
        result_dict = {}
        if use_space_group:
            _, sym_info = get_symmetry_info(crystal, tol=tol)
        ...
        material_id = _material_id_from_row(row) if "material_id" in row else None
        return {
            "mp_id": material_id,
            "_preprocess_failed": True,
            "_preprocess_error": f"{type(exc).__name__}: {str(exc)[:200]}",
        }
...
def preprocess(
    ...
    failures = [r for r in unordered_results if r.get("_preprocess_failed")]
    if failures:
        n_total = len(unordered_results)
        n_failed = len(failures)
        sample_errors = "\n".join(
            f"  {r.get('mp_id')}: {r.get('_preprocess_error')}" for r in failures[:5]
        )
        raise RuntimeError(
            f"preprocess(): {n_failed}/{n_total} rows failed pymatgen parsing.\n"
            f"First {min(5, n_failed)} failures:\n{sample_errors}\n"
            f"Fix the source CSV (re-curate or drop bad rows) and re-run."
        )
