...
def _cfg_triplet(
    ...
    value = _cfg_value(cli_val, model_args, key, default)
    if len(value) != 3:
        raise ValueError(f"{key} must have exactly 3 values, got {value}.")
...
def _resolve_sampler_name(
    cli_sampler: str | None,
    ckpt: dict[str, Any],
    model_args: dict[str, Any],
) -> str:
    raw = cli_sampler if cli_sampler is not None else ckpt.get("sampler", model_args.get("sampler", "edm"))
    name = str(raw).strip().lower()
    if name not in {"edm", "vfm", "catflow"}:
        raise ValueError(f"Unsupported sampler {name!r}. Use 'edm', 'vfm', or 'catflow'.")
    return name


def _catflow_predict_fn_for_checkpoint(model_args: dict[str, Any]):
    objective = str(model_args.get("training_objective", "")).strip().lower()
    impl = str(model_args.get("crystalite_impl_version", "")).strip().lower()
    if objective.startswith("catflow") or impl.startswith("crystalite_catflow"):
        raise RuntimeError(
            "catflow/vfm checkpoints are no longer supported on this branch "
            f"(src/vfm removed); got objective={objective!r}, impl={impl!r}."
        )
    return None
...
def _build_model_from_ckpt(
    *,
    ...
    if bool(model_args.get("discrete_types", False)):
        raise ValueError(
            "This codebase no longer supports Crystalite DNG checkpoints trained with "
            "discrete_types=True. Re-evaluate a continuous-EDM checkpoint or retrain "
            "without --discrete_types."
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--vfm_eps", type=float, default=None)
    parser.add_argument("--vfm_base_std_type", type=float, default=None)
    parser.add_argument("--vfm_base_std_coord", type=float, default=None)
    parser.add_argument("--vfm_base_std_lattice", type=float, default=None)
    parser.add_argument(
        "--vfm_time_grid",
        type=str,
        default=None,
        choices=["uniform", "power", "log_residual"],
    )
    parser.add_argument("--vfm_time_power", type=float, default=None)
    parser.add_argument(
        "--vfm_lattice_prior_mode",
        type=str,
        default=None,
        choices=["gaussian", "y1_physical"],
    )
    parser.add_argument("--vfm_lattice_prior_log_length_loc", type=float, nargs=3, default=None)
    parser.add_argument("--vfm_lattice_prior_log_length_scale", type=float, nargs=3, default=None)
    parser.add_argument("--vfm_lattice_prior_angle_min_deg", type=float, default=None)
    parser.add_argument("--vfm_lattice_prior_angle_max_deg", type=float, default=None)
    parser.add_argument(
        "--catflow_reference_odeint",
        action="store_true",
        help=(
            "CatFlow-only parity mode: ignore sample_num_steps and use torchdiffeq "
            "over t=[0, 1-vfm_eps] with fixed tolerances."
        ),
    )
    parser.add_argument("--sampler", type=str, default=None, choices=["edm", "vfm", "catflow", "vfm_velocity", "catflow_velocity"])
...
def main() -> None:
    ...
    sampler_name = _resolve_sampler_name(args.sampler, ckpt, model_args)
    catflow_predict_fn = _catflow_predict_fn_for_checkpoint(model_args)
    if catflow_predict_fn is not None and sampler_name == "catflow":
        print("[sample] Using EDM-preconditioned CatFlow predict function from checkpoint metadata.")
    ...
    vfm_eps = float(_cfg_value(args.vfm_eps, model_args, "vfm_eps", 1e-4))
    vfm_base_std_type = float(
        _cfg_value(args.vfm_base_std_type, model_args, "vfm_base_std_type", 1.0)
    )
    vfm_base_std_coord = float(
        _cfg_value(args.vfm_base_std_coord, model_args, "vfm_base_std_coord", 1.0)
    )
    vfm_base_std_lattice = float(
        _cfg_value(
            args.vfm_base_std_lattice,
            model_args,
            "vfm_base_std_lattice",
            1.0,
        )
    )
    vfm_time_grid = str(_cfg_value(args.vfm_time_grid, model_args, "vfm_time_grid", "uniform"))
    vfm_time_power = float(_cfg_value(args.vfm_time_power, model_args, "vfm_time_power", 2.0))
    vfm_lattice_prior_mode = str(
        _cfg_value(args.vfm_lattice_prior_mode, model_args, "vfm_lattice_prior_mode", "gaussian")
    )
    vfm_lattice_prior_log_length_loc = _cfg_triplet(
        args.vfm_lattice_prior_log_length_loc,
        model_args,
        "vfm_lattice_prior_log_length_loc",
        (1.5893, 1.7169, 1.9784),
    )
    vfm_lattice_prior_log_length_scale = _cfg_triplet(
        args.vfm_lattice_prior_log_length_scale,
        model_args,
    ...
    vfm_lattice_prior_angle_min_deg = float(
        _cfg_value(
            args.vfm_lattice_prior_angle_min_deg,
            model_args,
            "vfm_lattice_prior_angle_min_deg",
            60.0,
        )
    )
    vfm_lattice_prior_angle_max_deg = float(
        _cfg_value(
            args.vfm_lattice_prior_angle_max_deg,
            model_args,
            "vfm_lattice_prior_angle_max_deg",
            120.0,
        )
    )
    ...
    print(
        ...
        f"num_steps={sample_num_steps}, seed={sample_seed}, sampler={sampler_name}"
    )
    ...
    report = {
        "meta": {
            ...
            "vfm_eps": vfm_eps if sampler_name in {"vfm", "catflow"} else None,
            "vfm_base_std_type": (
                vfm_base_std_type if sampler_name in {"vfm", "catflow"} else None
            ),
            "vfm_base_std_coord": (
                vfm_base_std_coord if sampler_name in {"vfm", "catflow"} else None
            ),
            "vfm_base_std_lattice": (
                vfm_base_std_lattice
                if sampler_name in {"vfm", "catflow"}
                else None
            ),
            "vfm_time_grid": vfm_time_grid if sampler_name in {"vfm", "catflow"} else None,
            "vfm_time_power": vfm_time_power if sampler_name in {"vfm", "catflow"} else None,
            "catflow_reference_odeint": (
                bool(args.catflow_reference_odeint) if sampler_name == "catflow" else None
            ),
            "sample_seed": sample_seed,
